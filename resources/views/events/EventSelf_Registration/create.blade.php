    ...
    <div class="card event-card h-100 shadow-sm border-0">
        ...
        <div class="card-body">
            <h6 class="card-title text-truncate">{{ $event->event_name }}</h6>
                  <p class="small text-muted mb-2" style="font-weight: 500;">Total Cost (Inclusive of GST) : ₹{{ number_format($event->total_amount, 2) }}</p>
            <p class="small text-muted mb-2">
                {!! \Illuminate\Support\Str::limit(strip_tags($event->description), 70) !!}
            </p>
           <div class="text-center mt-3">
    <a href="#" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#registerModal{{ $event->id }}">
        View & Register
    </a>
</div>
...
<div class="modal fade" id="registerModal{{ $event->id }}" tabindex="-1" aria-hidden="true">
                ...
                <div class="col-lg-6">
                    <div class="bg-light rounded p-3 mb-3">
                        ...
                        <div class="d-flex justify-content-between small">
                            <span>Total Cost of Event(Inclusive of GST(INR))</span><span class="text-success">₹{{ number_format($event->total_amount, 2) }}</span>
                        </div>
                    </div>

                    
                            <?php $user = Auth::user(); ?>
                      <form action="{{ route('events.self_registraion_save') }}" method="POST" enctype="multipart/form-data">
                                 @csrf
                                <input type="hidden" name="event_id" value="{{ $event->id }}">

                                <div class="mb-2">
                              <label for="parent_id" class="form-label">Select Member</label>
                              <select class="form-select" name="parent_id" id="parent_id{{ $event->id }}">
    ...
    @foreach($allmembers as $parent)
        <option
            ...
            {{ old('parent_id', $member->parent_id ?? '') == $parent->id ? 'selected' : '' }}>
...
<script>
    $(function () {
        ...
        // scoped parent selector handling
        $('select[name="parent_id"]').on('change', function () {
            const container = $(this).closest('form');
            let selected = $(this).find(':selected');
            let name = selected.data('name') || '';
            let email = selected.data('email') || '';
            let contact = selected.data('contact') || '';

            container.find('input[name="name"]').val(name);
            container.find('input[name="email"]').val(email);
            container.find('input[name="contact_number"]').val(contact);
        });
    });
</script>
