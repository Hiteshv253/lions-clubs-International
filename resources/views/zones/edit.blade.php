            ...
            <div class="row g-3">
                <div class="col-sm-6 col-md-4">
                    ...
                    <select name="region_id" id="region_id" class="form-select @error('region_id') is-invalid @enderror" required>
                        ...
                        @foreach($regions as $region)
                            <option value="{{ $region->id }}" {{ old('region_id', $zone->region_id) == $region->id ? 'selected' : '' }}>
                                {{ $region->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-6 col-md-4">
                    ...
                    <select name="is_active" id="is_active"
                        ...
                        <option value="0" {{ old('is_active', $zone->is_active) == 0 ? 'selected' : '' }}>Active</option>
                        <option value="1" {{ old('is_active', $zone->is_active) == 1 ? 'selected' : '' }}>Inactive</option>
                    </select>
